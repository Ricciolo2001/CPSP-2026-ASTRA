...
class SLAM:
    ...
    params: dict
    n_particles: int
    system_noise_variance: np.ndarray
    ...
    def __init__(
        ...
        self.resampling_threshold = max(1, (n_particles * 10) // 100)
        self.current_state = current_state
        self.particles = np.zeros((4, n_particles))
        self.particles[:3, :] = current_state.reshape((3, 1)) * np.ones(
            (3, n_particles)
        )
        self.particles[3, :] = (1 / n_particles) * np.ones((1, n_particles))
    ...
    def update_state(self, ranges, angles, motion_update):
        ...
        """
        # motion model update
        self.particles[:3, :] += motion_update.reshape((3, 1)) * np.ones(
            (3, self.n_particles)
        )

        # State update: localize the current scan against the existing map.
        ...
        # Map update: fuse the current scan using the newly estimated pose.
