...
UartDaemon::UartDaemon(uart_port_t port, int baudrate, BleManager *ble)
    : _port(port), _baudrate(baudrate), _ble(ble), _taskHandle(NULL),
      _dataBuffer(NULL) {}
...
void UartDaemon::start(uint32_t stackSize, UBaseType_t priority) {
    if (_taskHandle == NULL) { // Check if the task is already running
...
void UartDaemon::run() {
    while (true) {
        int len = uart_read_bytes(_port, _dataBuffer, BUF_SIZE - 1,
        ...
        if (len > 0) {
            ...
            if (strcmp(command, "SCAN") == 0) {
                ...
                len = uart_read_bytes(_port, _dataBuffer, BUF_SIZE - 1,
                                      20 / portTICK_PERIOD_MS);
                if (len == 0) {
                    Serial.println("Failed to read");
                    uart_write_bytes(_port, "Failed to read",
