...
size_t uart_frame_encode(const uint8_t *payload, size_t payload_len,
    ...
    cobs_status_t status = cobs_encode(scratch, payload_len + CRC_SIZE, out_buf,
                                       out_max, &encoded_len);
...
bool uart_frame_decode(const uint8_t *frame, size_t frame_len, uint8_t *scratch,
    ...
    cobs_status_t status =
        cobs_decode(frame, cobs_input_len, scratch, scratch_size, &decoded_len);
    if (status != COBS_RET_OK) {
    ...
    uint16_t received_crc =
        (uint16_t)(((uint16_t)scratch[payload_len] << CRC_BYTE_SHIFT) |
                   (uint16_t)scratch[payload_len + 1]);
